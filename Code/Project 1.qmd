...
# Pretty print
kable(head(data_sort), format = "html") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```

No one has an IQ of 999. Likely a data entering error.
...
This table shows us that there appears to be a large difference in the mean IQ between the far (106) and Near (89.2) groups. However, looking at the boxplot, we can see that we clearly have a mistaken data entry with one child having an IQ of 999.
